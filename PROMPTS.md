...
```text
When generation_mode is similar, use base_reference as the single question to adapt. Keep its central mathematical methods, reasoning pattern and linked task structure; vary the numerical values and, where requested, its scenario. Do not combine unrelated source questions or merely paraphrase unchanged givens. Adapt the base to the requested question type, marks, parts, difficulty and selected syllabus scope, explaining any necessary configuration adjustment. Recompute every answer, distractor, diagram and marking allocation from the new givens. Other references are calibration evidence only. If candidate_context is present, use a distinct conceptual focus for each candidate while remaining an adaptation of its selected base. During independent review, reject an unchanged source question or a draft that no longer follows the selected base's central method. For refinements, previous and edit describe changes to the working draft; do not switch the base.
```
