...
import {z} from 'zod';
export const connectionSchema=z.object({provider:z.enum(['openai','azure','anthropic']).default('openai'),endpoint:z.string().max(250).default(''),deployment:z.string().max(100).default(''),model:z.string().max(100).regex(/^[a-zA-Z0-9._:-]*$/).optional()});
...
export async function providerError(r:Response,provider:string,key:string){
 ...
 return new ServiceResponseError(`${provider} request failed (${r.status})${code?` [${code}]`:''}${param?` (${param})`:''}: ${message||fallback}`,r.status>=500||r.status===429);
}
...
export function modelFor(c:Connection){return c.model|| (c.provider==='anthropic'?'claude-sonnet-5':c.provider==='azure'?c.deployment:'gpt-5.6-sol');}
...
export async function providerResponse(key:string,body:any,connection:Connection){
 ...
 const data:any=await readServiceJSON(r,'The AI provider');if(data.status==='incomplete'||data.stop_reason==='max_tokens')throw new Error('The model ran out of output space. Please simplify the request.');
