...
import {escapeXML as esc,svgDiagram} from './diagram';
import type {Draft} from './schema';
const mathRun=(t:string)=>`<m:r><m:rPr><m:sty m:val="p"/></m:rPr><m:t xml:space="preserve">${esc(t)}</m:t></m:r>`;
...
export function wordDocument(d:Draft,images:Uint8Array[],refs:string[]=[]){
 const files:Record<string,Uint8Array>={};let body=paragraph(d.title,'Title')+paragraph(`EM1 · ${d.total_marks} marks`)+paragraph('Question','Heading1')+paragraph(d.question);let rels='';
 const imageBlock=(i:number)=>{const diag=d.diagrams[i];files[`word/media/diagram${i}.svg`]=strToU8(svgDiagram(diag));if(!images[i])throw new Error('A diagram could not be prepared for Word.');files[`word/media/diagram${i}.png`]=images[i];rels+=`<Relationship Id="rP${i}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/diagram${i}.png"/><Relationship Id="rS${i}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/diagram${i}.svg"/>`;return `<w:p><w:r><w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0"><wp:extent cx="5486400" cy="3429000"/><wp:docPr id="${i+1}" name="Diagram ${i+1}" descr="${esc(diag.caption)}"/><a:graphic><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic><pic:nvPicPr><pic:cNvPr id="${i+1}" name="Diagram ${i+1}"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="rP${i}"><a:extLst><a:ext uri="{96DAC541-7B7A-43D3-8B79-37D633B846F1}"><asvg:svgBlip r:embed="rS${i}"/></a:ext></a:extLst></a:blip><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="5486400" cy="3429000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>`+paragraph(diag.caption);};
 d.diagrams.forEach((x,i)=>{if(x.placement==='question')body+=imageBlock(i);});
 d.solutions.forEach((s,i)=>{body+=paragraph(i===0?'Main solution':`Alternative solution ${i}`,'Heading1')+paragraph(s.title)+paragraph(s.content);if(i===0)d.diagrams.forEach((x,j)=>{if(x.placement==='solution')body+=imageBlock(j);});body+=paragraph('Proposed marking allocation','Heading2');s.marking.forEach(m=>body+=paragraph(`${m.part}: ${m.criterion} [${m.marks} marks]`));});
 ...
 files['word/document.xml']=xml(`<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture" xmlns:asvg="http://schemas.microsoft.com/office/drawing/2016/SVG/main"><w:body>${body}<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1134" w:right="1134" w:bottom="1134" w:left="1134"/></w:sectPr></w:body></w:document>`);
 files['word/styles.xml']=xml('<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:docDefaults><w:rPrDefault><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="22"/></w:rPr></w:rPrDefault></w:docDefaults><w:style w:type="paragraph" w:default="1" w:styleId="Normal"><w:name w:val="Normal"/></w:style><w:style w:type="paragraph" w:styleId="Title"><w:name w:val="Title"/><w:rPr><w:b/><w:sz w:val="34"/><w:color w:val="000000"/></w:rPr></w:style><w:style w:type="paragraph" w:styleId="Heading1"><w:name w:val="heading 1"/><w:pPr><w:keepNext/></w:pPr><w:rPr><w:b/><w:sz w:val="27"/></w:rPr></w:style><w:style w:type="paragraph" w:styleId="Heading2"><w:name w:val="heading 2"/><w:pPr><w:keepNext/></w:pPr><w:rPr><w:b/></w:rPr></w:style></w:styles>');
 files['word/_rels/document.xml.rels']=xml(`<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="styles" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>${rels}</Relationships>`);
 ...
 files['[Content_Types].xml']=xml('<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="png" ContentType="image/png"/><Default Extension="svg" ContentType="image/svg+xml"/><Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/><Override PartName="/word/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.styles+xml"/></Types>');return zipSync(files,{level:6});
}
