...
export function wordDocument(d:Draft,images:Uint8Array[],refs:string[]=[],module="Assessment"){
 const files:Record<string,Uint8Array>={};let body=paragraph(d.title,'Title')+paragraph(`${module} · ${d.total_marks} marks`)+paragraph('Question','Heading1')+paragraph(d.question)+d.parts.map(p=>paragraph(`${p.label} ${p.prompt}`)).join('')+d.options.map(o=>paragraph(`${o.label}. ${o.text}`)).join('');let rels='';
 const imageBlock=(i:number)=>{const diag=d.diagrams[i];if(!diag.graph)return wordShapes(diag,i+1,equation)+paragraph(diag.caption);if(!images[i]?.length)throw new Error('Prepare the Desmos graph image before exporting.');files[`word/media/graph${i}.png`]=images[i];rels+=`<Relationship Id="graph${i}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/graph${i}.png"/>`;return `<w:p><w:r><w:drawing><wp:inline><wp:extent cx="5486400" cy="3429000"/><wp:docPr id="${i+1}" name="Desmos graph" descr="${esc(diag.caption)}"/><a:graphic><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic><pic:nvPicPr><pic:cNvPr id="${i+1}" name="Desmos graph"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="graph${i}"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="5486400" cy="3429000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>`+paragraph(diag.caption);};
