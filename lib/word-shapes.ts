...
export function wordShapes(diagram:Draft['diagrams'][number],id:number,equation:(latex:string)=>string){
 const shapes=diagram.shapes.map((s,index)=>{
  ...
  if(isText){
   ...
   let content='';
   if(s.type==='math'||/^[A-Za-z]$/.test(s.text.trim()))content=`<m:oMathPara><m:oMathParaPr><m:jc m:val="left"/></m:oMathParaPr>${equation(s.text.replace(/^\$|\$$/g,''))}</m:oMathPara>`;
   else{let pos=0;for(const m of mathParts(s.text)){content+=plain(s.text.slice(pos,m.index))+equation(m.latex);pos=m.index+m.raw.length;}content+=plain(s.text.slice(pos));}
   ...
   const points=s.type==='line'||s.type==='arrow'||s.type==='measurement'?[{x:s.x,y:s.y},{x:s.x2,y:s.y2}]:s.points;
