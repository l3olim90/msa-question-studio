...
export async function generateMcqCandidates(key:string,raw:unknown,settings:Partial<Connection>={}){
 ...
 for(let index=0;index<3;index++){
  ...
  for(let attempt=0;attempt<2;attempt++){
   ...
   try{candidate=await generate(key,brief,undefined,'',settings,{index:index+1,prior:[...candidates.map(c=>c.draft),...attempts]});}catch(error){if(!(error instanceof DraftReviewError))throw error;lastReviewIssue=error.message;continue;}
