...
`app/library.tsx` provides repository management and worksheet assembly. `POST /api/worksheets` resolves the selected IDs and revisions against current approved entries, rejecting stale/deleted selections and duplicates. `worksheetDocument` reuses the individual DOCX renderer, creates unique image relationships/drawing IDs per question, preserves native equations/editable geometry, and includes named sections, continuous numbering, totals, optional identity fields, student instructions, a separate answer key and AI disclosure. Lecturer exports additionally include every main/alternative worked solution and marking allocation. Generated drafts include `answer_key`; legacy results default it to empty and use the main solution for their key. Assembly selections are not yet persisted.
