...
Reference requests use `lib/reference-request.ts` to key only inputs that affect retrieval: module, topic, subtopics, type, difficulty and specifications. Marks, part settings and creative-context flags do not restart checks. The browser caches successful checks for 60 seconds (up to 30 configurations), ignores cancelled responses and times out after 20 seconds with a retry action. The cloud bank uses one aggregate query rather than four queued queries and caches for 60 seconds, with explicit invalidation after imports. Other app instances and cached browser checks may take up to about two minutes to reflect remote imports. The hidden repository screen no longer loads data while generating questions.

The in-memory **Drafts this visit** list supports candidate paging and revisiting working drafts. Old `msa-question-history-v1` browser snapshots are read once for migration; new snapshots are no longer written. Only explicitly approved repository entries provide durable question storage. The existing history helpers remain for validated draft-list operations and legacy migration.
