...
const name=process.argv[2];if(!['check','providers-check','retry-check','migration-check','env-only-check','updates-check','features-check','bank','cloud','cloud-worker','cloud-verify'].includes(name))throw new Error('Unknown script.');
...
const env=testRun?{...process.env,STUDIO_STORAGE:'local',DATABASE_URL:'',SUPABASE_SECRET_KEY:'',VERCEL:''}:process.env;
