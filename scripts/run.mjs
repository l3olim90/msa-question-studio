...
const name=process.argv[2];if(!['check','providers-check','retry-check','migration-check','env-only-check','updates-check','features-check','team-check','similar-check','source-retry-check','security-check','database-connection-check','bank','cloud','cloud-worker','cloud-verify','repository-verify'].includes(name))throw new Error('Unknown script.');
