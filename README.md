...
Open **Session history** above the question preview to revisit any question generated in this browser tab. Each MCQ candidate is a separate question; refinements and manual diagram edits update its existing entry. History includes solutions, references and review results and survives refreshes using browser session storage. Export before closing the tab. Browser tab/session restoration may also restore history; it is not a permanent archive. If storage is blocked or full, the app shows a warning and retains history on the current page only.
...
Create a Langfuse project, then add these server settings to `.env` and restart:
...
Use the base URL for your project's region or self-hosted instance. Tracing records generation/refinement runs and nested provider calls with session/question identifiers, model, duration, token usage where returned, review outcomes, app release, and prompt date/hash. Content capture is off by default. Set `LANGFUSE_CAPTURE_CONTENT=true` only if sending prompt, source-reference and question text to that project is appropriate. Credentials, reference image bytes and hidden reasoning are excluded. Export failures produce a server warning and do not fail question generation. See [technical details and limitations](DOCUMENTATION.md#langfuse-audit-trail-and-monitoring).
