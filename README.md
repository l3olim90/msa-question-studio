...
The sub-topic checklist includes All, selecting every active sub-topic within the current topic. Changing the topic resets the selection. A provider-based feasibility check runs before generation and edits, using the selected notes and example rubrics. If the requested marks are insufficient, the UI explains the issue and suggests an estimated minimum; Use suggested marks updates the form without automatically spending credits on another generation. Conflicting specifications may require changes in addition to more marks. The estimate is advisory, not an official minimum. Generated drafts must match the requested total and include all selected syllabus IDs; the independent review checks actual coverage.
...
Alpha-feedback validation: tests cover empty/duplicate/cross-topic selections, all active sub-topics, integer marks including 1, exact-total enforcement, multi-selection generation, and infeasible configurations stopping before generation with a structured suggested minimum. Provider responses are mocked.
