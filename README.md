...
Choose **New question from the brief** or **Similar question from a source** before generating. Similar mode randomly chooses one compatible few-shot example and records its identity; it changes numbers/context while retaining the central mathematical method. It produces a separate draft. **Refine draft and recheck** changes the displayed draft instead.
...
In **Paper assembly**, add approved questions to named sections, reorder sections/questions or move questions between sections, and set the worksheet title, optional Name/Class fields, and free-text student instructions. Every **student Word** export contains the questions and an answer key. The separate **lecturer Word** export also contains all worked solutions and proposed marking allocations. Both include the AI generation disclaimer. Older questions without a concise answer key use the main worked solution as the key. Worksheet selections are temporary; approved questions remain saved in the repository.
